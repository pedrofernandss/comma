...
describe('Article Fetcher', () => {
  ...
  it('Should extract a summary from the "summary" field', async () => {
    const mockFeedWithSummary = {
      items: [
        {
          title: 'Article with summary',
          link: 'http://example.com/post-summary',
    ...
    (Parser as jest.Mock).mockImplementation(() => {
      return {
        parseURL: jest.fn().mockResolvedValue(mockFeedWithSummary),
      };
    });

    const article = await fetchLatestArticle('http://feed-with-summary/rss');
    expect(article?.summary).toBe('This is the summary field');
  });

  it('Should extract a summary from the "description" field', async () => {
    ...
    (Parser as jest.Mock).mockImplementation(() => {
      return {
        parseURL: jest.fn().mockResolvedValue(mockFeedWithDescription),
      };
    });

    const article = await fetchLatestArticle('http://feed-with-summary/rss');
    expect(article?.summary).toBe('This is the description field');
  });
});
